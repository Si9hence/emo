...
def collection(url, save=False):
    ...
    def get_data(url, save=False):
        ...
        for item in tmp:
            ...
            # the spectrum over file will be collected
                ...
                label = item.find(selector='h4')[0].text
                res['data'][label] = {'url':item.find(selector="img")[0].attrs['src'], 'upload':False}
        ...
        for datum in list(data['data'].keys()):
            if "Spectroscopic" in datum:
                ...
                for item in data['data'][datum]['files']:
                    path = repath(tmp['url'])
                    item['path'] = path
                if check_selected(datum, selected=None) == True:
                    data['data'][datum]['upload'] = True
                else:
                    data['data'][datum]['upload'] = False
        
        with open('{molecule}_{iso}_{dataset}.json'.format(molecule=data['molecule'], iso=data['isot'], dataset=data['dataset']), 'w') as f:
            json.dump(data, f)
    ...
    def emo_md(data:dict, flag="full"):
        ...
        reference_ExoMol = "**References:**<br>\n> 1. {ref}".format(ref=reference_base)
        ...
        url = data['url']
        des_ini = 'The dataset is an archive of ExoMol page, {url}.<br>Please check the reference details according to the following description or directly from the website.<br>\n'.format(url = url)
        file_name = "{molecule}_{isot}_{db}".format(molecule=molecule, isot=isot, db=db)
        msg_simple = '**NB: The html description skips data which are not included in the current version for the purpose of simplicity. Please check {file_name}.md for detailed information.**\n<br>'.format(file_name=file_name)
        msg_over = "**NB: These data are not included in the current version on Zenodo because the data are over Zenodo upload cap, 50GB**<br>\n**Data can be accessed via:** {url}<br>\n".format(url=url)
        ...
        if flag == 'full':
            with open("./{fn}.md".format(fn=file_name), 'w', encoding="utf-8") as fp:
                fp.write(des)
        elif flag == 'simple':
            with open("./{fn}.html".format(fn=file_name), 'w', encoding="utf-8") as fp:
...
def registration(data: dict, *, token: str = ''):
    ...
    def zenodo_upload(*, deposit_id: str, bucket_url: str, token: str):
        ...
        """
        file_description = "{molecule}_{isot}_{db}.md".format(molecule=molecule, isot=isot, db=db)
    ...
    def zenodo_metadata():
        ...
        def get_reference(data):
            ref = ["""Tennyson, J., Yurchenko, S. N., Al-Refaie, A. F., Clark, V. H. J., Chubb, K. L., Conway, E. K., Dewan, A., Gorman, M. N., Hill, C., Lynas-Gray, A. E., Mellor, T., McKemmish, L. K., Owens, A., Polyansky, O. L., Semenov, M., Somogyi, W., Tinetti, G., Upadhyay, A., Waldmann, I., Wang, Y., Wright, S., Yurchenko, O. P., "The 2020 release of the ExoMol database: molecular line lists for exoplanet and other hot atmospheres", J. Quant. Spectrosc. Rad. Transf., 255, 107228 (2020). \[[https://doi.org/10.1016/j.jqsrt.2020.107228](https://doi.org/10.1016/j.jqsrt.2020.107228)\]"""]
            for file in data['data']:
...
def zenodo_rec_deposit(token, path_save='./'):
    ...
    tmp = dict()
    for item in res:
        tmp[item['title']] = {'doi':item['links']['doi'],
